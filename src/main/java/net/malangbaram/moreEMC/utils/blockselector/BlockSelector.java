...
/**
 * @author hasunwoo A BlockSelector is a helper class that can request player to
 *         select one block in world.
 */
public class BlockSelector implements Closeable {
	...
	/**
	 * request for player's selection. when requested player hits any solid
	 * block, coordinate of that block will be recored once(if record was made,
	 * player will get removed from request queue) as well as player's world,
	 * which can be retrieved using getData. also, that event will cause
	 * BlockSelectedListener.onBlockSelected to fire.
	 * 
	 * @param player
	 */
	...
	/**
	 * if there is record stored, calling this method will cause BlockSelector
	 * to remove stored record of player.
	 * 
	 * @param player
	 * @return null if there is no data to retrieve, WorldPosition if there is
	 *         record.
	 */
	...
	/**
	 * this must be called when BlockSelector is no longer used. unregister from
